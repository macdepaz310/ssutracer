<?php
  ...
  if(!empty($_POST['email']) && !empty($_POST['pwd'])):
    ...
    $records = $conn->prepare('SELECT userid, adminuser, adminpassword FROM admin_user WHERE adminuser = :admin');
    $records->bindParam(':admin', $_POST['email']);
    $records->execute();
    $results = $records->fetch(PDO::FETCH_ASSOC);

    $message = '';

    if(count($results) > 0 && password_verify($_POST['pwd'], $results['adminpassword']) ){
      $_SESSION['currentUser'] = $results['userid'];
                                    ...
                                    <p><?= $message ?></p>
                                      <?php endif; ?>
                                        <!-- FORM FOR USER LOG IN -->
                                            <form class="form-horizontal" action="ssutraceradmin.php" method="post">
                                              <div class="form-group">
                                                  <label for="email" class="col-sm-2 control-label">Email</label>
                                                  <div class="col-sm-10">
                                                      <input type="email" name="admin" class="form-control" placeholder="example@gmail.com"  autofocus required>
                                                        </div>
                                                          </div>
                                                            <div class="form-group">
...
<div class="container-fluid">
  <div class="container">
      ...
      <div class="col-sm-3 well">
        ...
        <form class="form-horizontal" action="ssutraceradmin.php" method="post">
            <input type="text" name="admin" class="form-control" placeholder="example@gmail.com" required><br>
