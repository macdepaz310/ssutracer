...
try {
   ...
   $stmt = $conn->prepare("SELECT questionText FROM question_tbl where questionID=14");
   $stmt->execute();
...
if(isset($_POST['submit']) && !empty($_POST['ans'])):
  ...
  if($stmt->execute() ):
     header('Refresh: 0.2; url= ../survey/question15.php');
  else:
    echo "sorry";
  endif;
endif;
...
<html>
  ...
  <body>
    <form class="" action="question14.php" method="post">
      ...
      <input type="checkbox" name="ans2" value="Related to Course or Program of Study">Related to Course or Program of Study <br>
      <input type="checkbox" name="ans3" value="Proximity to Residence">Proximity to Residence <br>
      <input type="checkbox" name="ans4" value="other reason(s)">other reason(s) <br>
